...
# el mapping vive en data/entrada/presupuesto/aplicaciones a elementos de coste.xlsx
# y se carga a través de ContextoPresupuesto.aplicaciones_a_elementos_de_coste.
# Las filas con elemento_de_coste vacío o "xxx" son aplicaciones sin clasificar.
...
class TraductorPresupuesto:
    ...
    def _asignar_elemento_de_coste(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # Tabla de aplicación (4 dígitos) → elemento de coste cargada desde
        # data/entrada/presupuesto/aplicaciones a elementos de coste.xlsx.
        # Filas con elemento_de_coste vacío o "xxx" se tratan como sin clasificar.
        df_apl_full = self.ctx.aplicaciones_a_elementos_de_coste
        if df_apl_full is not None and not df_apl_full.is_empty():
            df_apl = (
                df_apl_full
                .filter(
                    pl.col("elemento_de_coste").is_not_null()
                    & (pl.col("elemento_de_coste") != "xxx")
                    & (pl.col("elemento_de_coste").str.strip_chars() != "")
                )
                .select(
                    pl.col("aplicación"),
                    pl.col("elemento_de_coste").alias("_ec_apl"),
                )
            )
            df = df.join(df_apl, on="aplicación", how="left")
        else:
            df = df.with_columns(pl.lit(None).cast(pl.Utf8).alias("_ec_apl"))
