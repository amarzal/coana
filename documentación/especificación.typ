...
#table(
    ...
    [*Regla 23*],
    [#campo("jornada_anual_pdi") · #campo("factor_impartición_docente") · #campo("sexenio_vivo_años") · #campo("másteres_ficticios_pod") · #campo("umbral_residual_regla23")],
    [*Tesis*],
...
Por convención, un nodo dinámico #etqact("X-{proyecto}") cuelga de su actividad base #etqact("X"). Hay tres emplazamientos *no convencionales* fijados a propósito: los nodos #etqact("ait-financiación-externa-XXX") cuelgan de #etqact("transf-otras"); los #etqact("ai-financiación-propia-XXX") cuelgan de #etqact("otras-ait-financiación-propia"); y los de transferencia del cargador de proyectos (#etqact("transf-XXX") y #etqact("transf-60-XXX")) cuelgan de #etqact("transf-60") (ver §«Cargador proyectos»).
...
#nota[El grupo de investigación de cada proyecto se *deriva* de los contratos (proyecto→contrato→IP→grupo del IP donde es principal/coordinador), con desempate al grupo único y fallback al departamento; un fichero opcional #ruta("proyectos a grupos.xlsx") permite corregir los casos ambiguos. Ver §«Lo que es de INVES» y #ruta("coana", "fase1", "grupo_por_proyecto.py"). Una tabla oficial proyecto→IP→grupo seguiría siendo preferible a la derivación.]
    ...
    - #nombre-regla("No son ayudas")
        ...
        - #nombre-regla[Proyectos singulares]
            ...
            #align(center, table(
                ...
                val("8G033"),
                etqact("dag-escuela-doctorado"),
                [TESIS DOCTORALES — presupuestariamente en departamentos por reparto de cargas, pero conceptualmente DAG de la Escuela de Doctorado. Además, su centro de coste se fuerza a #etqcen("ed") (ver «Centro forzado por proyecto» en el clasificador de centros), de modo que el reparto dag lo lleva a los programas de doctorado.],
...
#reglas[
    ...
    - #nombre-regla[Por servicio existe servicio y el proyecto uno de la línea]
        ...
        #table(
            ...
            [251], [Servei d'Activitats Socioculturals], [#etqcen("sasc")], [#etqact("dag-cultura")],
            [760], [Servei d'Esports], [#etqcen("se")], [#etqact("dag-deportes")],
    ...
    - #nombre-regla[Centro forzado por proyecto] (máxima prioridad)
        Algunos proyectos se ejecutan presupuestariamente en un centro (departamento, facultad…) por conveniencia de gestión, pero su coste pertenece *conceptualmente* a otro centro. Para esos, una tabla #campo("proyecto → centro de coste") (#campo("_CC_POR_PROYECTO") en #ruta("coana", "fase1", "clasificador_centros_coste.py")) fija el centro de coste *por encima de cualquier otra regla* —es el primer nivel del coalescado, antes incluso de suministros y cátedras—, con independencia del #campo("centro") y #campo("subcentro") del apunte. Las filas afectadas se marcan en la traza con #campo("regla_cc") = #val("[Centro forzado por proyecto]").

        *Caso actual — Tesis doctorales (#val("8G033")).* El proyecto #val("8G033") (TESIS DOCTORALES) reparte su gasto entre los departamentos por una cuestión pragmática de reparto de cargas, pero conceptualmente es una actividad de la DAG de la *Escuela de Doctorado*. Por eso se le fuerza el centro de coste a #etqcen("ed") y, vía #campo("_PROYECTOS_ESPECIALES"), la actividad a #etqact("dag-escuela-doctorado"). El efecto combinado: como la base finalista propia del centro #etqcen("ed") son los programas de doctorado, la fase de reparto de actividades distribuye ese coste *entre los programas de doctorado* (subárbol #etqact("doctorado")) en lugar de derramarlo a los grados y másteres de los departamentos donde se ejecutó.

    - #nombre-regla[Proyecto de investigación INVES → grupo de investigación del IP]
        Si el #campo("centro") es #val("INVES"), no es cátedra y el proyecto resuelve a un grupo de investigación (ver más abajo), el centro de coste es #etqcen("grupo-investigación-<id_grupo>"). Tiene prioridad sobre la regla del #campo("centro_origen"). Así el gasto de los proyectos de investigación se imputa al grupo del IP, en línea con la dedicación investigadora de la regla 23 (que ya va al grupo).

        Mientras no exista la tabla propia proyecto→IP→grupo, el grupo se *deriva* encadenando los ficheros de #ruta("entrada", "investigación"): #ruta("proyectos en contratos investigación.xlsx") (proyecto↔contrato) → #ruta("investigadores en contratos.xlsx") (contrato↔IP, #campo("principal")=#val("S")) → #ruta("investigadores en grupos.xlsx") (grupo del IP, activo en el año y donde es #campo("principal")/#campo("coordinador")), validando contra #ruta("grupos a institutos.xlsx"). *Desempate:* el conjunto de grupos «principal/coordinador» de los IP del proyecto debe ser exactamente uno; si es 0 ó >1, no se resuelve y cae a la regla del #campo("centro_origen") (departamento). Un fichero opcional #ruta("proyectos a grupos.xlsx") (#campo("proyecto"), #campo("id_grupo")) *sustituye* lo derivado (#campo("id_grupo") vacío elimina la asignación). Implementado en #ruta("coana", "fase1", "grupo_por_proyecto.py").

    - #nombre-regla[Lo que es de INVES debe ir al centro_origen]
        Si el #campo("centro") es #val("INVES") y el proyecto *no* resuelve a un grupo (regla anterior), el centro de coste es el que resulta de aplicar la TABLA-TRADUCCIÓN-DEPARTAMENTOS al #campo("centro_origen") del proyecto (se traduce el código del centro presupuestario al identificador del centro de coste correspondiente, p. ej. #val("DADEM") → #etqcen("daem"), #val("DFICO") → #etqcen("dfc")).
...
*PTGAS — tipo 8.* De #ruta("entrada", "nóminas", "reducciones laborales.xlsx"), histórico de todas las reducciones laborales, tomamos solo las filas con #campo("tipo reduccion") = #val("8") (representación sindical) que solapan el año analizado. Las demás (lactancia, cuidado de hijos, etc.) se tratan aparte como *absentismo* (ver «Reducciones de jornada no sindicales» más abajo).
...
En ambos casos la suma de #campo("horas_finales") de la persona es exactamente su jornada disponible ($"jornada_anual_pdi" times "fracción_año"$, repartida entre la dedicación sindical y el resto). Para el PDI #emph[sin] reducción sindical, un exceso de docencia + gestión sobre la jornada sigue siendo una anomalía que se reporta sin corregir.
...
Estos nodos se insertan dinámicamente en el árbol de actividades: los de *transferencia* —#etqact("transf-60-XXX") (art. 60) y #etqact("transf-XXX") (resto de anexos tipo 1)— cuelgan de #etqact("transf-60") (Contratos artículo 60 y similares); los demás (#etqact("ai-nacional-XXX"), #etqact("cátedras-aulas-empresa-XXX")…) cuelgan de su actividad base.
...
+ *Lista de personas* (master): per_id, persona, *% docencia*, *% gestión*, *% investigación* y *% jornada cubierta* (calculados sobre las horas finales tras el reparto), nº actividades y nº filas con anomalía. Los tres primeros se calculan sobre la *jornada efectiva* de la persona (la suma de sus horas finales) y suman #val("100 %") salvo en casos anómalos (exceso de docencia + gestión, asociados sin docencia registrada). El *% jornada cubierta* es, en cambio, la fracción de la jornada anual completa (#val("1 642 h")) realmente cubierta: #val("100 %") para quien trabaja todo el año y menos si la relación es parcial (incorporación o jubilación a mitad de año).
...
+ *Reparto por grupo y origen* (panel resumen al seleccionar persona): una fila por cada par (grupo, origen) con horas registradas, factor medio, horas iniciales efectivas, horas finales tras el reparto y porcentaje sobre la *jornada efectiva* de la persona (la suma de sus horas finales, que es la jornada anual prorrateada por el período trabajado). Los porcentajes suman #val("100 %"): no hay fila #emph[Sin asignación], porque el reparto en cascada ya cubre exactamente la jornada disponible (el tiempo no contratado —p. ej. tras una jubilación a mitad de año— no es dedicación sin asignar, simplemente no forma parte de la jornada). La cabecera de cada columna de horas muestra además su suma total.
+ *Detalle por actividad*: las filas de #campo("dedicación_pdi") para la persona, con #campo("origen_id"), #campo("método"), #campo("factor"), las *horas finales* repercutidas y el *% jornada* de cada actividad concreta, además de las anomalías. Las filas sintéticas con #campo("origen") = #val("reparto") aparecen al final cuando ha sido necesario crear una actividad #etqact("ai") para absorber las horas no distribuidas que la cascada imputa a investigación.
...
#table(
    ...
    [#etqcen("dpeesm") · Psicologia Evolutiva, Educativa, Social i Metodologia],
    [#etqact("grado-psicología")],
    [#etqcen("fcs")],
    table.hline(),
)
...
El modelo de contabilidad analítica exige que los costes de *gestión agregada* —las UC cuya *actividad* es de tipo #emph[dag] (subárbol de #etqact("dags"), códigos #val("02.*")): departamentos, vicerrectorados, servicios generales, institutos, amortizaciones genéricas…— se repartan entre las *actividades finalistas* (subárbol de #etqact("principales"), códigos #val("01.*")). Es una fase posterior e independiente de la fase 1, con su propio botón *Reparto actividades* en la barra lateral; toma como entrada el combinado #ruta("data", "fase1", "unidades de coste.xlsx") y produce sus propios artefactos y visores. El módulo vive en #ruta("coana", "reparto").

== Modelo de reparto

Cada UC tiene tres coordenadas: *centro de coste*, *actividad* y *elemento de coste* (C-A-E). El reparto solo toca las UC cuya *A es dag* (subárbol de #etqact("dags")); *conserva su elemento de coste E* y reparte el importe entre actividades *finalistas* (subárbol de #etqact("principales")), generando *fragmentos*.

=== Patrones

Las reglas se escriben con *patrones de etiqueta* sobre los códigos del árbol. Un patrón es una de tres formas:

- #val("*") — cualquier nodo.
- #etq("etiqueta") — *exactamente* ese nodo.
- #etq("etiqueta") seguido de #val(".*") — ese nodo *y todo su subárbol*.

P. ej. #val("dag-escuela-doctorado.*") casa #etqact("dag-escuela-doctorado") y sus descendientes; #val("dags.*") casa cualquier dag; #val("doctorado.*") todas las finalistas de doctorado. El patrón se *resuelve por etiqueta → código → comparación de prefijo* (la etiqueta se traduce a su código y se compara con el código del nodo). Como las reglas se escriben con *identificadores estables* (no con códigos literales), reordenar el árbol *no* las rompe.

El patrón de *centro destino* admite además el comodín relacional #val("·mismo·"): el centro del *propio origen* y su subárbol. Permite expresar «repártelo dentro de su propio centro» sin nombrarlo.

=== Reglas

Una *regla* es #campo("ORIGEN(actividad, centro) → DESTINO(actividad, centro)"). En cada lado, tanto la actividad como el centro pueden ser *uno o varios* patrones; varios patrones significan la *unión* de sus conjuntos (p. ej. destino actividad #val("docencia.* + ai.*") = todas las finalistas de docencia ∪ investigación).

- *Match del ORIGEN:* una UC dag casa la regla si su #campo("actividad") casa *algún* patrón de actividad origen *y* su #campo("centro") casa *algún* patrón de centro origen.
- *Prelación:* las reglas son una *lista ordenada*; gana la *primera* cuyo ORIGEN casa (legible para humanos: se ordenan de específica a general y el atrapalotodo va al final). No hay puntuación por especificidad.
- *DESTINO:* la UC origen se reparte entre las *UC no-dag (finalistas)* que casan el DESTINO (su actividad casa algún patrón de actividad destino *y* su centro algún patrón de centro destino; #val("·mismo·") se resuelve con el centro del origen), *en proporción al importe de cada destino*.

#campo("importe(fragmento) = importe_origen × importe(destino) / Σ importe(destinos)")

*Fragmento.* Cada fragmento *imita la naturaleza del destino* (toma su #campo("centro") y su #campo("actividad")) pero *conserva el elemento de coste del origen* y se anota con #campo("marca_dag") = la actividad dag de procedencia. Lleva #campo("origen") = #val("reparto-dag") y #campo("origen_porción") = la fracción repartida. Las UC no-dag *no se mueven*. Para acotar el número de fragmentos, las UC dag se agregan por #campo("(centro, actividad, elemento de coste)") antes de repartir.

*Destino vacío → anomalía (o materialización).* Si la primera regla que casa el origen tiene un *conjunto de destinos vacío* (ninguna UC no-dag casa el DESTINO, o suman importe #val("0")), la UC dag *no se reparte*: se conserva intacta y se lista como *anomalía* —no se cae a la siguiente regla—. Cada anomalía se estudia y se le da una regla a medida. *Excepción:* una regla puede llevar el flag #campo("materializar"), y entonces, en vez de anomalía, *crea* el destino nombrado (las actividades de #campo("destino_actividad"), sin el #val(".*")) en el centro destino, *a partes iguales*, aunque no existiera ninguna UC allí. Es para los *grupos e institutos de investigación sin actividad finalista propia*: su coste es su investigación propia, que se materializa como #etqact("otras-ait-financiación-propia") en el propio centro. La conservación global es siempre exacta: #campo("Σ post-reparto = Σ entrada").

== Tabla de reglas de reparto DAGs

La *Tabla de reglas de reparto DAGs* es un *artefacto de diseño* (no proviene de la base de datos corporativa): la definimos al especificar el sistema. Vive como literal en #ruta("coana", "reparto", "tabla_dag_centro.py") (#campo("REGLAS")) y se documenta aquí. Es una *lista ordenada*; gana la *primera* cuyo ORIGEN casa, y la última es el atrapalotodo por defecto.

El *defecto* (última fila) reparte cada dag entre las finalistas de su *propio centro* (#val("·mismo·")): el caso habitual de facultades, departamentos, escuelas e institutos (una dag en #etqcen("estce") va a los grados y másteres de la ESTCE). Por encima, reglas más específicas desvían lo que no debe quedarse en su centro. Consecuencias notables del modelo:

- Los *costes generales* en la *raíz* #etqcen("cc-uji") (primas de seguros, gastos financieros, material de oficina, tributos…) los reparte el propio defecto: como #val("·mismo·") de la raíz es *todo su subárbol* (todos los centros), se distribuyen a *todas* las finalistas. No hace falta regla especial.
- Las *tesis doctorales* y demás coste de #etqact("dag-escuela-doctorado"), caiga en el centro que caiga, se dirigen a los *programas de doctorado* (#etqact("doctorado.*")) en #etqcen("ed").
...
#table(
    ...
    etqact("dag-fcje.*"), etqcen("*"), etqact("estudios-oficiales.*"), etqcen("fcje.*"),
    etqact("dag-fcs.*"), etqcen("*"), etqact("estudios-oficiales.*"), etqcen("fcs.*"),

    // Conserjerías
    etqact("dag-conserjería-estce"), etqcen("*"), etqact("estudios-oficiales.*"), etqcen("estce.*"),
    etqact("dag-conserjería-fcje"), etqcen("*"), etqact("estudios-oficiales.*"), etqcen("fcje.*"),
    etqact("dag-conserjería-fchs"), etqcen("*"), etqact("estudios-oficiales.*"), etqcen("fchs.*"),
    etqact("dag-conserjería-fcs"), etqcen("*"), etqact("estudios-oficiales.*"), etqcen("fcs.*"),
...
Una UC dag cuya *primera regla que casa* deja un *destino vacío* (ninguna UC no-dag casa los patrones de destino, o suman importe #val("0")) *no se reparte*: se conserva intacta (para no perder coste) y se lista en el visor de anomalías con su motivo (#val("destino_vacío")). No se cae a la siguiente regla. El visor de anomalías es la *herramienta de diseño* de la Tabla de reglas de reparto DAGs: muestra los pares #campo("(centro, actividad)") pendientes para darles una regla a medida (por encima del defecto). El visor *Por actividad dag* complementa: permite pinchar cada dag repartida y ver el detalle de su reparto (a qué finalistas y centros fue, con importe y %); y, pinchando un destino, los *fragmentos individuales* (una UC por elemento de coste, con su importe, porción y #campo("origen_id")) que componen ese destino.
...
#table(
    ...
    val("2K2"), etqact("otras-ayudas-estudiantes"), val("10"),
    val("2PC"), etqact("ai-internacional"), val("10"),
    val("2A0"), etqact("otras-ait-financiación-propia"), val("3"),
    val("2M*"), etqact("otras-ait-financiación-propia"), val("3"),
    val("2PE"), etqact("ai-internacional"), val("10"),
    val("2PN"), etqact("ai-nacional"), val("10"),
    val("2PV"), etqact("ai-regional"), val("10"),
    val("2PA"), etqact("ai-nacional"), val("10"),
...
#table(
    ...
    [#campo("es_asociado")],
    [Boolean],
    [La persona es una figura puramente docente (associat PAA/PAL o substitut PS) en el año. El nombre de la columna se conserva por compatibilidad histórica.],
    [#campo("sexenio_vivo")],
    [Boolean],
